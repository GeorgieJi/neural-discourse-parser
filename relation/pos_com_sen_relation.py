...
def train_with_sgd(model,train_samples,index_to_word=[],index_to_relation=[]):
    ...
    for i in np.random.permutation(len(train_samples)):
        ...
        print 'the ce error of this batch : ' , ce_err
...
def sgd_updates_adadelta(norm,params,cost,rho=0.95,epsilon=1e-8,norm_lim=9,word_vec_name='Words'):
    """
...
def preprocess(pairs):
    ...
    pairslines = []
    for pair in pairs:
        pairslines.append(pair[1])
        pairslines.append(pair[2])


    print 'now preprocessing ... '
    with open('tmp/pairs','w') as f:
        for l in pairslines:
            f.write(l.strip()+'\n')
    ...
    # print cmd
    os.system(cmd)
    pairspos = pos_parser('tmp/pairs','tmp/pairs.tag')
    return pairspos
...
def build_data(dir_path,maxlen):
    ...
    for pair in pairs:
        ...
        n = n+1
        # 
        mv_pairs.append([pair[0], p_1,p_2])
    ...
    for mv_pair in mv_pairs:
        ...
        p1sen = p1sen.strip().replace('<P>',' p_end ')
        p2sen = p2sen.strip().replace('<P>',' p_end ')

        # pair[1] = pair[1].strip().replace('<P>','')
        # pair[2] = pair[2].strip().replace('<P>','')

        wrdsa = p1sen.strip().lower().split()
        wrdsb = p2sen.strip().lower().split()

        possa = p1pos.strip().split()
        possb = p2pos.strip().split()
        ...
        rel = relation_mapping(rel.lower()).lower()
    ...
    return s1sens , s2sens , s1poss, s2poss, disrels
...
class gru_tensor:
    ...
    def recurrent(self,x_s,x_m,E_):
        ...
        def _step_slice(m_, x_, xx_, h_, U, Ux):
            ...
            r = T.nnet.sigmoid(_slice(preact,0,hidden_dim))
            u = T.nnet.sigmoid(_slice(preact,1,hidden_dim))
...
def load_data():
    ...
    maxlen = 50
    ledus, redus , lpos, rpos, rels = build_data('../data/RSTmain/RSTtrees-WSJ-main-1.0/TRAINING',maxlen);
    tst_ledus, tst_redus, tst_lpos, tst_rpos, tst_rels = build_data('../data/RSTmain/RSTtrees-WSJ-main-1.0/TEST',maxlen)
    ...
    pos_list = []
    for posa, posb in zip(lpos,rpos):
        pos_list.extend(posa)
        pos_list.extend(posb)
    ...
    # collect all POS information
    pos_freq = nltk.FreqDist(pos_list)
    print 'Found %d unique pos tokens . '% len(pos_freq.items())
    ...
    X_1_train_pos = np.asarray([[pos_to_index[w] for w in sent ] for sent in lpos])
    X_2_train = np.asarray([[word_to_index[w] for w in sent ] for sent in redus])
    X_2_train_pos = np.asarray([[pos_to_index[w] for w in sent ] for sent in rpos])
    X_1_test = np.asarray([[word_to_index[w] for w in sent ] for sent in tst_ledus])
    X_1_test_pos = np.asarray([[pos_to_index[w] for w in sent ] for sent in tst_lpos])
    X_2_test = np.asarray([[word_to_index[w] for w in sent ] for sent in tst_redus])
    X_2_test_pos = np.asarray([[pos_to_index[w] for w in sent ] for sent in tst_rpos])
    ...
    X_1_train_pos, X_1_train_posmask = prepare_data(X_1_train_pos,maxlen)
    X_2_train_pos, X_2_train_posmask = prepare_data(X_2_train_pos,maxlen)
    y_train = (rels)
    ...
    X_1_test_pos, X_1_test_posmask = prepare_data(X_1_test_pos,maxlen)
    X_2_test_pos, X_2_test_posmask = prepare_data(X_2_test_pos,maxlen)
    y_test = (tst_rels)

    print 'build_data'
    print len(X_1_train)
    print len(X_2_train)
    print len(X_1_train_pos)
    print len(X_2_train_pos)
    ...
    print "Example sentence '%s' " % " ".join(lpos[0])
    print "Example sentence '%s' " % " ".join(rpos[0])

    print "Example sentence after Pre-processing : '%s' " % X_1_train[0]
    print "Example sentence after Pre-processing : '%s' " % X_2_train[0]
    print "Example sentence after Pre-processing : '%s' " % X_1_train_pos[0]
    print "Example sentence after Pre-processing : '%s' " % X_2_train_pos[0]
    print "Example label : ", y_train[0]
    print ""

    # the pos mask is the same as sentence word mask
    train_set = [X_1_train, X_1_train_mask, X_2_train, X_2_train_mask, X_1_train_pos, X_2_train_pos, y_train]
    test_set = [X_1_test, X_1_test_mask, X_2_test, X_2_test_mask, X_1_test_pos, X_2_test_pos, y_test]


    return train_set, test_set, index_to_word, word_to_index, vocabulary_size, index_to_relation, relation_to_index, index_to_pos, pos_to_index, maxlen
...
class framework:
    ...
    """
    def  __init__(self,word_dim,pos_dim,label_dim,vocab_size,pos_size,maxlen,hidden_dim=128,word_embedding=None,POS_embedding=None,bptt_truncate=-1):
        ...
        p_1 = T.matrix('p_1',dtype='int64')
        p_2 = T.matrix('p_2',dtype='int64')
        ...
        # the frameword only holds the global POS embedding 
        ...
        # POS tensor layer
        f_gru_1 = gru_tensor(pos_dim,hidden_dim,bptt_truncate=-1)
        b_gru_1 = gru_tensor(pos_dim,hidden_dim,bptt_truncate=-1)
        ...
        mlp_layer = HiddenLayer(hidden_dim*8,label_dim)
        ...
        self.params += f_gru_1.params
        self.params += b_gru_1.params
...
def relation():
    ...
    # load data set
    train_set , test_set , index_to_word, word_to_index, vocabulary_size, index_to_relation, relation_to_index, index_to_pos, pos_to_index, maxlen = load_data()
    X_1_train, X_1_train_mask, X_2_train, X_2_train_mask, X_1_train_pos, X_2_train_pos, y_train = train_set
    X_1_test, X_1_test_mask, X_2_test, X_2_test_mask, X_1_test_pos, X_2_test_pos, y_test = test_set
    ...
    model = framework(word_dim,pos_dim,label_size,vocabulary_size,pos_size,maxlen,hidden_dim=hidden_dim,word_embedding=E,POS_embedding=P,bptt_truncate=-1)
    ...
    for epoch in range(NEPOCH):
        ...
        # test_score(model,train_samples,index_to_word=index_to_word,index_to_relation=index_to_relation)
